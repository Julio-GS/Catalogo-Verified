...
export default function iMac({ products }) {
  const iMac = products.filter((product) => product.Categoría === "iMac");
  ...
  const filters = [
    {
      name: "stock",
      label: "Stock",
      options: [...new Set(iMac.map((product) => product.Stock))],
    },

    {
      name: "capacity",
      label: "Capacidad",
      options: [...new Set(iMac.map((product) => product.Capacidad))],
    },
  ];

  useEffect(() => {
    setResults(iMac);
    setFilteredResults(iMac);
  }, [products]);
