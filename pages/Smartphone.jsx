...
const Smartphone = ({ products }) => {
  ...
  const [filteredResults, setFilteredResults] = useState(smartphone);

  const filters = [
    {
      name: "marca",
      label: "Marca",
      options: [...new Set(smartphone.map((product) => product.Marca))],
    },
    {
      name: "capacity",
      label: "Capacidad",
      options: [...new Set(smartphone.map((product) => product.Capacidad))],
    },
  ];

  useEffect(() => {
    setResults(smartphone);
    setFilteredResults(smartphone);
  }, []);
