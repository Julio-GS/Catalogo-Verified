      ...
      <section className="py-24 w-full">
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="w-full ">
            <Image
              src={mainImage || data[0]?.imgPrincipal}
              alt="Producto"
              width={500}
              height={500}
            ...
            <div className="flex gap-4 mt-4 w-full">
              {/* Asegúrate de incluir la imagen principal actual en las opciones si deseas que sea seleccionable nuevamente */}
              <Image
                src={data[0]?.imgPrincipal}
                alt="Producto"
                width={150}
                height={150}
                className={`rounded-lg shadow-lg ${
                  mainImage === data[0]?.imgPrincipal
                    ? "border-2 border-gray-500"
                    : ""
                }`}
                onClick={() => handleImageClick(data[0]?.imgPrincipal)}
              />
              <Image
                src={data[0]?.imgFrente}
                alt="Producto"
                width={150}
                height={150}
                className={`rounded-lg shadow-lg ${
                  mainImage === data[0]?.imgFrente
                    ? "border-2 border-gray-500"
                    : ""
                }`}
                onClick={() => handleImageClick(data[0]?.imgFrente)}
              />
              <Image
                src={data[0]?.imgCostado}
                alt="Producto"
                width={150}
                height={150}
                className={`rounded-lg shadow-lg ${
                  mainImage === data[0]?.imgCostado
                    ? "border-2 border-gray-500"
                    : ""
                }`}
                onClick={() => handleImageClick(data[0]?.imgCostado)}
              />
            </div>
          </div>
          <div>
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600">{data[0]?.Descripcion}</p>
            </div>
            <div className="mb-8">
              <h3 className="text-lg font-medium text-gray-800 mb-2">
            ...
            <div className="mb-8">
              ...
              <div className="flex space-x-4 select-none">
                {data[0]?.Talles.split(" ").map((talle, index) => (
                  <label key={index}>
                    <input
                      type="radio"
                      name="type"
